...
class ClimateIntegrationConfigurator(IntegrationConfigurator):
    ...
    def configure(self, config):
        ...
        payload = payload_template_json.format(prefix = self._prefix, node = self._node, obj = self._object)
        print("payload: ", payload)
        self._mqttc.publish(topic, payload='{"device_class": "battery", "name": "Temperature", "state_topic": "homeassistant/sensor/sensorBedroom/state", "unit_of_measurement": "°C", "value_template": "{{ value_json.temperature}}" }', qos=1, retain=False)
